...
export class CactusCustodianApi extends EventEmitter implements ICustodianApi {
  ...
  async getEthereumAccounts(): Promise<IEthereumAccount<ICactusEthereumAccountCustodianDetails>[]> {
  ...
  async signTypedData_v4(
    ...
    const result = await this.client.signTypedData_v4(address, message, version, message.domain?.chainId);
