...
describe("BitgoCustodianApi", () => {
  ...
  beforeEach(() => {
    bitgoCustodianApi = new BitgoCustodianApi({ jwt: mockJwt }, AuthTypes.TOKEN, mockUrl, 0);

    mockedBitgoClientInstance = mockedBitgoClient.mock.instances[0];
    mockedBitgoClientInstance.getEthereumAccounts = jest.fn().mockImplementation(() => bitgoGetAccountsMock.data);

    mockedBitgoClientInstance.getTransactions = jest.fn().mockImplementation(() => [bitgoTransactionMock]);

    mockedBitgoClientInstance.getTransaction = jest.fn().mockImplementation(() => bitgoTransactionMock);

    mockedBitgoClientInstance.createTransaction = jest.fn().mockImplementation(() => bitgoTransactionMock);

    mockedBitgoClientInstance.getCustomerProof = jest.fn().mockImplementation(() => bitgoGetCustomerProofMock);

    mockedBitgoClientInstance.getEthereumAccountByAddress = jest
      .fn()
      .mockImplementation(() => bitgoGetAccountsMock.data[0]);

    mockedBitgoClientInstance.signPersonalMessage = jest.fn().mockImplementation(() => bitgoMockPersonalSignResponse);

    mockedBitgoClientInstance.signTypedData_v4 = jest.fn().mockImplementation(() => bitgoMockEip712Response);
  ...
  describe("BitgoCustodianApi#getEthereumAccountsByAddress", () => {
    it("fetches the ethereum accounts for a given address", async () => {
      const result = await bitgoCustodianApi.getEthereumAccountsByAddress("0xed");

      expect(result.length).toBe(bitgoGetAccountsMock.data.filter(acc => acc.address.startsWith("0xed")).length);
    });
  });
  ...
  describe("BitgoCustodianApi#getEthereumAccountsByLabelOrAddressName", () => {
    it("fetches the ethereum accounts based on label or address name", async () => {
      const result = await bitgoCustodianApi.getEthereumAccountsByLabelOrAddressName("MMI Alpha test 1");

      expect(result.length).toBe(
        bitgoGetAccountsMock.data.filter(acc => acc.labels[0].value.includes("MMI Alpha test 1")).length,
      );
    });
  });
  ...
  describe("BitgoCustodianApi#createTransaction", () => {
    ...
    it("throws if the ethereum account does not exist when we search by wallet", async () => {
      mockedBitgoClientInstance.getEthereumAccounts = jest.fn().mockImplementationOnce(() => []);
  ...
  describe("BitgoCustodianApi#signPersonalMessage", () => {
    it("calls signPersonalMessage on the client", async () => {
      ...
      const result = await bitgoCustodianApi.signPersonalMessage(fromAddress, buffer);
      ...
      expect(mockedBitgoClientInstance.signPersonalMessage).toHaveBeenCalledWith(
  ...
  describe("BitgoCustodianApi#signTypedData_v4", () => {
    it("calls client.signTypedData_v4", async () => {
      ...
      const result = await bitgoCustodianApi.signTypedData_v4(fromAddress, buffer, "V4");
  ...
  describe("BitgoCustodianApi#getSupportedChains", () => {
    it("calls getEthereumAccounts on the client and returns the chainId for that specific account", async () => {
      ...
      expect(mockedBitgoClientInstance.getEthereumAccountByAddress).toHaveBeenCalledWith("0x");

      expect(result).toEqual([bitgoGetAccountsMock.data[0].chainId.toString()]);
    });

    it("should return null if the account isnt found", async () => {
      mockedBitgoClientInstance.getEthereumAccountByAddress = jest.fn().mockImplementationOnce(() => null);
