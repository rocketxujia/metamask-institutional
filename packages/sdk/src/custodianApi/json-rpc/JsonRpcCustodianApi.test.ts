...
describe("JsonRpcCustodianApi", () => {
  ...
  beforeEach(() => {
    jsonRpcCustodianApi = new JsonRpcCustodianApi({ refreshToken: mockJwt }, AuthTypes.TOKEN, mockUrl, 0);

    mockedJsonRpcClientInstance = mockedJsonRpcClient.mock.instances[0];
    mockedJsonRpcClientInstance.listAccounts = jest.fn().mockImplementation(() => mockJsonRpcListAccountResponse);
    ...
    mockedJsonRpcClientInstance.signTypedData = jest.fn().mockImplementation(() => mockJsonRpcSignTypedDataResponse);

    mockedJsonRpcClientInstance.signPersonalMessage = jest.fn().mockImplementation(() => mockJsonRpcSignResponse);

    mockedJsonRpcClientInstance.getAccountChainIds = jest
      .fn()
      .mockImplementation(() => mockJsonRpcListAccountChainIdsResponse);

    mockedJsonRpcClientInstance.on = jest.fn().mockImplementation(() => mockJsonRpcListAccountChainIdsResponse);
  ...
  describe("getEthereumAccountByAddress", () => {
    it("gets the accounts, and then filters by address", async () => {
      const result = await jsonRpcCustodianApi.getEthereumAccountsByAddress(fromAddress);
  ...
  describe("getEthereumAccountsByLabelOrAddressName", () => {
    it("gets the accounts, and then filters by account name", async () => {
      const result = await jsonRpcCustodianApi.getEthereumAccountsByLabelOrAddressName("Elegantly Jittery Viper Fish");
  ...
  describe("createTransaction", () => {
    it("calls create transaction on the client (EIP-1559)", async () => {
      ...
      expect(mockedJsonRpcClientInstance.createTransaction).toHaveBeenCalledWith([
        {
          from: fromAddress,
          to: txParams.to,
  ...
  describe("getTransaction", () => {
    it("calls getTransaction on the client", async () => {
      const result = await jsonRpcCustodianApi.getTransaction(null || "", mockJsonRpcGetTransactionByIdPayload[0]);

      expect(mockedJsonRpcClientInstance.getTransaction).toHaveBeenCalledWith(mockJsonRpcGetTransactionByIdPayload);

      expect(result).toEqual({
        custodian_transactionId: mockJsonRpcGetTransactionByIdResponse.result.id,
        transactionStatus: mapStatusObjectToStatusText(mockJsonRpcGetTransactionByIdResponse.result.status),
        transactionStatusDisplayText: mockJsonRpcGetTransactionByIdResponse.result.status.displayText,
        ...
        maxPriorityFeePerGas: mockJsonRpcGetTransactionByIdResponse.result.maxPriorityFeePerGas,
  ...
  describe("signTypedData_v4", () => {
    it("should call signTypedData on the client", async () => {
      ...
      expect(mockedJsonRpcClientInstance.signTypedData).toHaveBeenCalledWith(mockJsonRpcSignTypedDataPayload);
  ...
  describe("signPersonalMessage", () => {
    it("should call signPersonalMessage on the client", async () => {
      ...
      expect(mockedJsonRpcClientInstance.signPersonalMessage).toHaveBeenCalledWith(mockJsonRpcSignPayload);
    ...
    it("should throw an error if the address does not exist", async () => {
      expect(jsonRpcCustodianApi.signPersonalMessage("fake", mockJsonRpcSignPayload[1])).rejects.toThrowError(
  ...
  describe("getSupportedChains", () => {
    it("should call getAccountChainIds on the client", async () => {
      ...
      expect(mockedJsonRpcClientInstance.getAccountChainIds).toHaveBeenCalledWith(["0xtest"]);
  ...
  describe("getTransactionLink", () => {
    it("should call getTransactionLink on the client", async () => {
      const result = await jsonRpcCustodianApi.getTransactionLink(mockJsonRpcGetTransactionLinkPayload[0]);

      expect(mockedJsonRpcClientInstance.getTransactionLink).toHaveBeenCalledWith(mockJsonRpcGetTransactionLinkPayload);
