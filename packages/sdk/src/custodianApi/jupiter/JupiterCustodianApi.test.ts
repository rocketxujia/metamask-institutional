import { AuthTypes, ITransactionDetails } from "@mm-institutional/types";
...
describe("JupiterCustodianApi", () => {
  ...
  beforeEach(() => {
    jupiterCustodianApi = new JupiterCustodianApi({ jwt: mockJwt }, AuthTypes.TOKEN, mockUrl, 0);

    mockedJupiterClientInstance = mockedJupiterClient.mock.instances[0];
    mockedJupiterClientInstance.getEthereumAccounts = jest.fn().mockImplementation(() => jupiterAccountsMock);

    mockedJupiterClientInstance.getTransactions = jest.fn().mockImplementation(() => [jupiterTransactionMock]);

    mockedJupiterClientInstance.getTransaction = jest.fn().mockImplementation(() => jupiterTransactionMock);

    mockedJupiterClientInstance.createTransaction = jest.fn().mockImplementation(() => jupiterTransactionMock);

    mockedJupiterClientInstance.getCustomerProof = jest.fn().mockImplementation(() => jupiterCustomerProofMock);

    mockedJupiterClientInstance.signTypedData_v4 = jest.fn().mockImplementation(() => jupiterCreateEIP712SignatureMock);

    mockedJupiterClientInstance.signPersonalMessage = jest
      .fn()
      .mockImplementation(() => jupiterCreatePersonalSignatureMock);

    mockedJupiterClientInstance.getSupportedChains = jest.fn().mockImplementation(() => ({ "4": "rinkeby" }));
  ...
  describe("JupiterCustodianApi#getEthereumAccounts", () => {
    it("returns the accounts", async () => {
      ...
      expect(mockedJupiterClientInstance.getEthereumAccounts).toHaveBeenCalled();
  ...
  describe("JupiterCustodianApi#getEthereumAccountsByAddress", () => {
    it("fetches the ethereum accounts for a given address", async () => {
      const result = await jupiterCustodianApi.getEthereumAccountsByAddress("0xb");

      expect(result.length).toBe(jupiterAccountsMock.filter(acc => acc.address.startsWith("0xb")).length);
    });
  });
  ...
  describe("JupiterCustodianApi#getEthereumAccountsByLabelOrAddressName", () => {
    it("fetches the ethereum accounts based on label or address name", async () => {
      const result = await jupiterCustodianApi.getEthereumAccountsByLabelOrAddressName("Impressive");

      expect(result.length).toBe(jupiterAccountsMock.filter(acc => acc.label.includes("Impressive")).length);
    });
  });
  ...
  describe("JupiterCustodianApi#createTransaction", () => {
    it("finds the account, then calls client.createTransaction", async () => {
      ...
      expect(mockedJupiterClientInstance.createTransaction).toHaveBeenCalledWith(
        {
    ...
    it("throws if the ethereum account does not exist when we search by wallet", async () => {
      mockedJupiterClientInstance.getEthereumAccounts = jest.fn().mockImplementationOnce(() => []);
  ...
  describe("JupiterCustodianApi#getTransaction", () => {
    it("gets all the transactions and filters them", async () => {
      const result: ITransactionDetails = await jupiterCustodianApi.getTransaction("0x", jupiterTransactionMock.id);
      ...
      expect(mockedJupiterClientInstance.getTransaction).toHaveBeenCalledWith(jupiterTransactionMock.id);
    });
  });
  ...
  describe("JupiterCustodianApi#signPersonalMessage", () => {
    it("finds an ethereum address, then calls client.signPersonalMessage", async () => {
      ...
      const result = await jupiterCustodianApi.signPersonalMessage(fromAddress, buffer);
      ...
      expect(mockedJupiterClientInstance.signPersonalMessage).toHaveBeenCalledWith(jupiterAccountsMock[0].id, buffer);
    });
    ...
    it("throws if the ethereum account does not exist when we search by wallet", async () => {
      ...
      expect(jupiterCustodianApi.signPersonalMessage(fromAddress, buffer)).rejects.toThrow("No such ethereum account");
    });
  });
  ...
  describe("JupiterCustodianApi#signTypedData_v4", () => {
    it("finds an ethereum address, then calls client.signTypedData_v4", async () => {
      ...
      const result = await jupiterCustodianApi.signTypedData_v4(fromAddress, buffer, "V4");
    ...
    it("throws if the ethereum account does not exist when we search by wallet", async () => {
      ...
      expect(jupiterCustodianApi.signTypedData_v4(fromAddress, buffer, "V4")).rejects.toThrow(
        "No such ethereum account",
      );
    });
  });
  ...
  describe("JupiterCustodianApi#getCustomerProof", () => {
    it("calls getCustomerProof on the client and returns the token", async () => {
      ...
      expect(mockedJupiterClientInstance.getCustomerProof).toHaveBeenCalledWith("jupiter-customer", "some_website");
    });
  });
